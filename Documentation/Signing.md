...
The Arcade SDK include a set of predefined configurations for the SignTool in the [Sign.proj](../src/Microsoft.DotNet.Arcade.Sdk/toolset/Sign.proj) file. However, you can override/remove/update any of these configurations by including a file named `Signing.props` in the `\eng\` folder of your repository. See examples on the next section.
...
Click [here](../src/Microsoft.DotNet.Arcade.Sdk/toolset/Sign.proj) to see how the Arcade SDK calls the `SignToolTask`. Here's a snippet:
...
1. Update the existing `FileExtensionSignInfo` and `StrongNameSignInfo` metadata in [Sign.props](../src/Microsoft.DotNet.Arcade.Sdk/toolset/Sign.props)
