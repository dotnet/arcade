...
| `GitHubAppServiceConnection` | `'dnceng-oneloc-githubapp'` | The dnceng/internal WIF service connection used to sign the App JWT. When left at its default, Arcade automatically uses `devdiv-oneloc-githubapp` in `DevDiv/DevDiv`; explicit overrides are preserved for separately provisioned infrastructure. |
