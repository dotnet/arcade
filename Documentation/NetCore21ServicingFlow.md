...
### Azure DevOps Pipelines and Pipebuild Definitions

The pipebuild descriptions for the builds are held in https://dev.azure.com/devdiv/DevDiv/_git/DotNet-BuildPipeline.  The definitions are comprised of an outer pipebuild definition which orchestrates inner pipebuild and utility pipelines.  For example, the orchestrated pipebuild pipeline launches the corefx pipebuild pipeline with specific parameters, waits for it to complete, then launches the corefx pipebuild pipeline.
- **Public Servicing** - Public builds will use publicly visible intermediate storage and publish build information to the dotnet/versions repo on completion.
  - **Azure DevOps Pipeline** - https://dev.azure.com/devdiv/DevDiv/_build/index?definitionId=8287
...
- **Internal Servicing** - Internal builds will use private intermediate storage, including private blob feeds and will not publish build information to the dotnet/versions repo on completion.
  - **Azure DevOps Pipeline** - https://dev.azure.com/devdiv/_build/index?definitionId=9010
...
5. Perform **initial** validation builds with pre-release branding (e.g. date varying)
    - **If public** - Launch public servicing pipeline with the default parameters and monitor progress.  Monitoring can be done via the Azure DevOps Pipelines console or mission control.
    - **If internal** - Launch internal servicing pipeline with the default parameters and monitor progress.  Monitoring can be done via the Azure DevOps Pipelines console or mission control.
6. Perform any partial build respins as necessary (e.g. failures, new fixes, etc.) - New builds can be partially spun starting from a specific repo.  To do this, requeue the pipeline with the following parameters:
...
7. Validate outputs based on pipeline input parameters
    - **If public** - Check dotnet/versions repo once pipeline for README containing links to build outputs, including blob feed.  Raw outputs can be found in `$(PB_FeedBaseUrl)/$(ProductBuildId)/final`
...
- What kind of changes have been made to the pipelines or repositories lately?  Prodcon v1 treats each repository's build infrastructure as a black box.  Each of them only needs to conform to the following contract:
    ...
    If significant changes have been made to pipelines or repo infrastructure code, ensuring that this contract is still abided by can ensure a proper isolated product build.
