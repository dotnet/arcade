...
4. CodeQL 3000 requires a specific configuration file to convey TSA Bug Filing configuration. To use TSA Bug Filing, copy Arcade's [`tsaoptions.json`](../.config/tsaoptions.json), updating the `areaPath`, `notificationAliases`, `repositoryName` and `codebaseName` to values appropriate for your repository. Note that your repository is likely already using TSA for other compliance tooling, and those values may be used here.
