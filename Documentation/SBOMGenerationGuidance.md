...
- `BuildDropPath` : Determines the directory that the SBOM tooling will use to find build outputs.
  Defaults to $`(Build.Repository.LocalPath)/artifacts` to match Arcade's convention. 
...
- If the SBOM generation task fails with the message:
  ...
  `$(Build.Repository.LocalPath)/Artifacts`. In this case you should modify the `BuildDropPath`
