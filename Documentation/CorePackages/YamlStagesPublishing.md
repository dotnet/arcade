...
to.  For more information on channels, see the [Channels, Branches and Subscriptions document](../BranchesChannelsAndSubscriptions.md).
    ...
    **Note:** At the moment, triggering stages manually is not supported by Azure DevOps. Once this capability
    ...
    Once these properties are set, the Arcade SDK will publish build artifacts to the Build's Artifacts,
    and the post-build stages will move the assets in the artifacts to the final feeds. The artifacts that
    the post-build templates use are:
