...
[string]$installScriptsVersion = if (Test-Path variable:installScriptsVersion) { $installScriptsVersion } else { "v1" }
...
function GetDotNetInstallScript([string] $dotnetRoot) {
  ...
  if (!(Test-Path $installScript)) {
    ...
    Invoke-WebRequest "https://dot.net/$installScriptsVersion/dotnet-install.ps1" -OutFile $installScript
  }
