...
foreach ($subscription in $subscriptions) {
    if ($subscription.enabled -and $subscription.policy.updateFrequency -eq 'everyBuild') {
...
foreach ($subscriptionToTrigger in $subscriptionsToTrigger) {
    try {
        $triggerSubscriptionApiEndpoint = "$maestroEndpoint/api/subscriptions/$subscriptionToTrigger/trigger?api-version=2018-07-16"
        $headers = Get-Headers 'application/json' $BarToken
        
        Write-Host "Triggering subscription '$subscriptionToTrigger'..."

        $postResponse = Invoke-WebRequest -Uri $triggerSubscriptionApiEndpoint -Headers $headers -Method Post
