...
namespace Microsoft.DotNet.Build.Tasks.Feed.Tests
{
    ...
    public class PushToBuildStorageTests
    {
        ...
        /// </summary>
        /// <param name="pushToLocalStorage"></param>
        [Theory]
        [InlineData(true)]
        [InlineData(false)]
        ...
        public void V4_WithPipelineArtifactNamesShouldEmitFutureAssetLocationsInManifest()
        {
            ...
            model.Artifacts.Packages[0].PipelineArtifactPath.Should().Be("TestFutureArtifactName/nonshipping-packages/test-package-a.6.0.492.nupkg");
            model.Artifacts.Packages[1].PipelineArtifactPath.Should().Be("TestFutureArtifactName/shipping-packages/test-package-b.6.0.492.nupkg");
            // Note that blobs are copied to the output directory based on their relative blob path,
            // rather than the original blob path
            model.Artifacts.Blobs[0].PipelineArtifactPath.Should().Be("TestFutureArtifactName/blobs/path/to/blob/file.zip");
        }
        ...
        public void V4_FuturePathsShouldOnlyUseForwardSlash()
        {
            ...
            // Validate that the future artifact name is set correctly
            model.Artifacts.Pdbs[0].PipelineArtifactPath.Should().Be("TestFutureArtifactName/pdbs/path/to/a/pdb.pdb");
            model.Artifacts.Pdbs[1].PipelineArtifactPath.Should().Be("TestFutureArtifactName/pdbs/path/to/b/pdb.pdb");
