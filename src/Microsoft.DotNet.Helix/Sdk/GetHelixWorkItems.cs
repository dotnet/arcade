...
namespace Microsoft.DotNet.Helix.Sdk
{
    public class GetHelixWorkItems : HelixTask
    {
        ...
        private async Task<IEnumerable<ITaskItem>> GetWorkItemsAsync(ITaskItem job, CancellationToken cancellationToken)
        {
            ...
            if (status.Working > 0)
            {
                Log.LogError($"This task can only be used on completed jobs. There are {status.Working} of {status.Total} unfinished work items.");
            ...
            IDictionary<string, string> CreateWorkItemMetadata(string name)
            {
                ...
                var consoleUri = HelixApi.BaseUri.AbsoluteUri.TrimEnd('/') + $"/api/2019-06-17/jobs/{jobName}/workitems/{Uri.EscapeDataString(name)}/console";
