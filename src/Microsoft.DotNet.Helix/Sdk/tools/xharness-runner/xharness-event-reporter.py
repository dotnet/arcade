...
def analyze_operation(command: str, platform: str, isDevice: bool, target: str, targetOS: str, exitCode: int):
    ...
    if platform is "android":
        if exitCode is 85: # ADB_DEVICE_ENUMERATION_FAILURE
            ...
            print('Encountered ADB_DEVICE_ENUMERATION_FAILURE. This is typically not a failure of the work item. We will run it again and reboot this computer to help its devices')
            ...
            if not isDevice:
                # Copy emulator log
                subprocess.Popen(['cp', '/tmp/*-logcat.log', output_directory]).wait()

            reboot = True
            retry = True
        if exitCode is 78: # PACKAGE_INSTALLATION_FAILURE
...
for operation in operations:
    ...
    analyze_operation(command, platform, isDevice, target, targetOS, exitCode)
