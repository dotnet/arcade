...
public class LocalTestResultsReaderTests
{
    ...
    private readonly ILogger<LocalTestResultsReader> _logger =
    ...
    public async Task LocalTestResultsReader_DoesNotAttachEmptyOutput(string format)
    {
        ...
        var reader = CreateReader(TestResultAttachmentMode.All);
    ...
    public async Task LocalTestResultsReader_CombinesPackedAndXmlResultsAcrossWorkItems()
    {
        ...
        IReadOnlyList<TestResult> resultSets = await CreateReader().ReadResultFileAsync(filePath);
        IReadOnlyList<AggregatedResult> aggregate = new ResultAggregator().Aggregate([resultSets]);
    ...
    private LocalTestResultsReader CreateReader(TestResultAttachmentMode attachmentMode = TestResultAttachmentMode.Failed)
        => new(_logger, attachmentMode, _fileSystem);

    private string WriteResultFile(string directory, string format, string outcome, bool includeOutput)
    {
        ...
        _fileSystem.WriteToFile(filePath, result.Content);
        return filePath;
    }
