...
namespace Microsoft.DotNet.EntityFrameworkCore.Extensions
{
    public static class SystemVersionedSqlServerEntityTypeBuilderExtensions
    {
        public static ModelBuilder ForSqlServerIsSystemVersioned<TEntity, THistoryEntity>(
            [NotNull] this ModelBuilder modelBuilder, string retentionPeriod)
            where TEntity : class
            where THistoryEntity : class
        {
            var historyBuilder = modelBuilder.Entity<THistoryEntity>();
            var tableBuilder = modelBuilder.Entity<TEntity>();
            historyBuilder.Metadata.SetAnnotation(DotNetExtensionsAnnotationNames.HistoryTable, true);
            tableBuilder.Metadata.SetAnnotation(DotNetExtensionsAnnotationNames.SystemVersioned, historyBuilder.Metadata.Name);
            ...
            historyBuilder.Property<DateTime>("SysStartTime")
                .HasColumnType("datetime2")
                .ValueGeneratedOnAddOrUpdate();
            historyBuilder.Property<DateTime>("SysEndTime")
                .HasColumnType("datetime2")
                .ValueGeneratedOnAddOrUpdate();
