...
namespace Maestro.Web
{
    ...
    {
        public GitHubAuthenticationHandler(IOptionsMonitor<GitHubAuthenticationOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock) : base(options, logger, encoder, clock)
        {
        }
        ...
        protected override async Task<AuthenticationTicket> CreateTicketAsync(ClaimsIdentity identity, AuthenticationProperties properties, OAuthTokenResponse tokens)
        {
            ...
            using (var response = await Backchannel.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, Context.RequestAborted))
            {
                ...
                identity.AddClaim(new Claim(ClaimTypes.NameIdentifier, payload.Value<string>("id"), ClaimValueTypes.String, Options.ClaimsIssuer));
                identity.AddClaim(new Claim(ClaimTypes.Name, payload.Value<string>("login"), ClaimValueTypes.String, Options.ClaimsIssuer));
                var email = payload.Value<string>("email");
                if (email != null)
                {
                    identity.AddClaim(new Claim(ClaimTypes.Email, email, ClaimValueTypes.String, Options.ClaimsIssuer));
                }
                identity.AddClaim(new Claim("urn:github:name", payload.Value<string>("name"), ClaimValueTypes.String, Options.ClaimsIssuer));
                identity.AddClaim(new Claim("urn:github:url", payload.Value<string>("url"), ClaimValueTypes.String, Options.ClaimsIssuer));
