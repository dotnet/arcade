    ...
    public class CommandLineParser
    {
        ...
        public static void ParseForConsoleApplication(Action<CommandLineParser> parseBody)
        {
            Parse(parseBody, parser =>
            {
                ...
                var helpString = parser.GetHelpString(maxLineWidth);
                ShowHelp(helpString);
                Environment.Exit(0);
        ...
        public string GetHelpString(int maxLineWidth)
        {
            // We allow  <parameterSet> /? and /? <parameterSet> to limit help 
            var helpString = string.Empty;
            if (args.Count == 2)
            {
                var parameterSet = args[0] ?? args[1];
                if (parameterSet != null)
                {
                    if (IsQualifier(parameterSet))
                        parameterSet = parameterSet.Substring(1);     // remove the -
                    helpString = GetHelpForParameterSet(parameterSet, true, maxLineWidth);
                }
            }
            if (helpString.Length == 0)
                helpString = GetHelp(maxLineWidth);
        ...
        {
            get { return lastQualifierWins; }
            set { lastQualifierWins = value; }
        }
        ...
        private void ParseWords()
        {
            ...
            for (int i = 0; i <= commandLine.Length; i++)
            {
                ...
                else if (c == ' ' || c == '\t' || c == '\0')
                {
                    if (wordStartIndex >= 0)
                    {
                        // AddWord may append some more chars to commandline in the case of a response file
                        AddWord(wordStartIndex, i, numQuotes, hasExcapedQuotes, isResponseFile);
                        wordStartIndex = -1;
        ...
        private void AddWord(int wordStartIndex, int wordEndIndex, int numQuotes, bool hasExcapedQuotes, bool isResponseFile)
        {
            if (!readExe)
            {
                readExe = true;
        ...
        private int GetNextOccuranceQualifier(string name, string[] aliases)
        {
            ...
            if (aliases != null)
            {
                ...
                {
                    int aliasEncodedPos;
                    if (dashedParameterEncodedPositions.TryGetValue(alias, out aliasEncodedPos))
                    {
        ...
        /// no next instance of the qualifier.  
        /// </summary>
        private object DefineQualifier(string name, Type type, object defaultValue, string helpText, bool isRequired)
        {
            ...
            for (; ; )
            {
                ...
                else if (occuranceCount > 0 && !lastQualifierWins)
        ...
        private bool lastQualifierWins;
        ...
        private bool skipParameterSets;             // Have we found the parameter set qualifier, so we don't look at any others.   
