...
bool cs = false, vb = false, fs = false;
...
foreach (var node in XDocument.Load(resxFile).Descendants("data"))
{
    ...
    {
        strings.Append($"{memberIndent}{(cs || fs ? "///" : vb ? "'''" : throw null)} ");
    ...
    {
        strings.AppendLine($"    let {identifier} = ResourceManager.GetString(\"{name}\", Culture)");
    }
    else
    {
        throw null;
...
namespace {namespaceName ?? "SR"}

open System.Reflection

module {className} =
    type private C (_dummy:System.Object) = class end
    let mutable Culture = System.Globalization.CultureInfo.CurrentCulture
    let ResourceManager = new System.Resources.ResourceManager(""{resourceName}"", C(null).GetType().GetTypeInfo().Assembly)
