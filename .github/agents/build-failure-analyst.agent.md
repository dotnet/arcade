...
The caller (typically `build-failure-analysis.md` or `build-failure-analysis-command.md`) locates the failed **Azure DevOps** `arcade-pr` build, downloads the `.binlog` each build leg produced (it does **not** rebuild), uploads them as an artifact, and the gh-aw MCP gateway mounts them read-only into the `binlog-mcp` container under the directory `/data/binlogs` (one `*.binlog` per leg, enumerated in `GH_AW_BINLOG_LIST`). The caller also sets the environment variables below. You must read all of them before doing anything else.
...
| MSBuild task / target failure | `MSB####` | Missing file, malformed XML, broken import, broken Arcade SDK target. |
| NuGet resolution failure | `NU####`, `NETSDK####` | Package not found, version conflict, TFM not supported, banned dependency, or a version not yet mirrored to `dotnet-public`. **Use the NuGet MCP server** to resolve. |
...
> **Note:** The NuGet MCP server operates on the workspace's actual project files and NuGet configuration. It has access to the repository's NuGet feeds and can resolve transitive dependency chains that are impossible to reason about from error messages alone. In Arcade, a `NU1102`/`NU1100` frequently means the package version exists on nuget.org but has not yet been mirrored to the `dotnet-public` feed (see `Documentation/MirroringPackages.md`) — flag that as a likely cause rather than proposing a downgrade.
...
| ---- | ------- | --------- | ------- |
| `CS0103` | `Microsoft.DotNet.Arcade.Sdk` | `Foo.cs:42` | The name 'foo' does not exist… |
...
- Do not propose fixes to files outside the PR diff in scan mode unless you are extremely confident — those changes are usually load-bearing across other projects (Arcade SDK targets/props are consumed by the whole .NET org). Prefer to explain the root cause in the comment and let a human apply the fix.
...
- Use the project's preferred terms (e.g., `Arcade`, `Arcade SDK`, `eng/common`, `Helix SDK`) instead of generic phrasing.
