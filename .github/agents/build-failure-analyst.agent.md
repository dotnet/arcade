...
4. Every file you touch is under `src/`. The workflow's `allowed-files` allowlist refuses anything else, and build infrastructure (`eng/`, `global.json`, `NuGet.config`, `.github/`) must never be "fixed" this way.
...
3. Commit with a first line naming the fix and the marker on its own line, e.g.:
   ...
   ```text
   Fix CS1503 after Microsoft.DotNet.ProductConstructionService.Client bump

   GetBuildAsync gained an includeAssetLocation parameter before
   cancellationToken, so pass the token by name at both call sites.
...
- Do not propose fixes to files outside the PR diff in scan mode unless you are extremely confident — those changes are usually load-bearing across other projects (Arcade SDK targets/props are consumed by the whole .NET org). Prefer to explain the root cause in the comment and let a human apply the fix. The single exception is Step 6b, whose conditions (mechanical fix, provable from the compiler error, `src/` only, same-repo PR, no previous automated attempt) exist precisely to keep that confidence bar high.
