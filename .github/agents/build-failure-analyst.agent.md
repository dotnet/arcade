...
> **These `binlog_*` functions are MCP tools** exposed by the `binlog-mcp` server — call them **directly as tools**, passing a JSON argument object (e.g. `binlog_errors` with `{ "binlog_file": "<path>" }`). There is **no** `binlog-mcp` shell executable; never try to run `binlog-mcp …` via `bash`/`shell` — the shell allowlist excludes it and those calls are denied. The binlog files are only readable through these MCP tools, not via `cat`/`ls` on `/data/binlogs`.
