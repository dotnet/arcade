...
3. Load your detailed playbook: `cat .github/agents/build-failure-analyst.agent.md`
   ...
   - When the fix belongs to a file the PR never touched, an inline
     ...
     the fix in the summary comment. You do not need to check whether an
     automated fix ran before — a trusted job already refused to start this run
     if the branch *tip* is itself an automated fix.
