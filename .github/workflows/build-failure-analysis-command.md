...
description: >-
  ...
  `arcade-pr` build, downloads the binary logs that build already produced
    ...
    steps:
      - name: Download binlogs from the PR's latest failed Azure Pipelines build
        ...
        run: |
          ...
          # --- Find the PR's most recent COMPLETED arcade-pr build (merge ref) ---
          ...
          echo "Latest completed arcade-pr build for PR #${PR_NUMBER}: id='${BUILD_ID}' result='${BUILD_RESULT}'"
          [ -z "${BUILD_ID}" ] && { echo "::warning::No completed arcade-pr build found for PR #${PR_NUMBER}."; emit_none; }
          if [ "${BUILD_RESULT}" != "failed" ]; then
            echo "::warning::PR #${PR_NUMBER}'s latest arcade-pr build (${BUILD_ID}) result is '${BUILD_RESULT}', not failed — the failure looks resolved; nothing to analyse."
            emit_none
          fi
