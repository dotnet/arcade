...
# trusted fetch job (step 3c) and reads only the subject of the PR's *tip*
...
tools:
  ...
  bash:
    ...
    # `git log` is deliberately absent too: the checkout is depth-1 so it can
    # only ever show the tip, and the loop guard it might be mistaken for lives
    # in the trusted fetch job.
    - "git status:*"
    - "git diff:*"
    - "git rev-parse:*"
    - "git add:*"
